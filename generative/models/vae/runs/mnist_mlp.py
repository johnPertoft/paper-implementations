...
if __name__ == "__main__":
    ...
    model_name = VAE.__name__
    log_dir = os.path.join("log_dir", model_name, run_name)
    results_dir = os.path.join("results", model_name, run_name)

    model_path = run_training(vae, log_dir, n_training_steps=settings["N training steps"])
    with restored_session(model_path) as sess:
        vae.generate_results(sess, results_dir, list(settings.items()))
